...
{
    FILE *logFile = fopen("ux0:/sonic4ep2_LOG.log", "a+");
    if (logFile != NULL) {
        fprintf(logFile, "%s\n", fmt);
        fclose(logFile);
    }
...
void _write_to_logfile(const char *fi, int li, const char *fn, const char* fmt, ...)
{
    FILE *logFile = fopen("ux0:/sonic4ep2_LOG.log", "a+");
    if (logFile != NULL) {
        fprintf(logFile, "LOG DATA: %s %d %s %s\n", fi, li, fn, fmt);
        fclose(logFile);
    }
